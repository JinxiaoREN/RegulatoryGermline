...
while read lines; do
	...
	echo "gcloud alpha genomics pipelines run --pipeline-file gatk_germline.yaml --inputs fafile=gs://dinglab/reference/GRCh37-lite.fa,faifile=gs://dinglab/reference/GRCh37-lite.fa.fai,bamfile=${bam},baifile=${bai},id=${id},chrlist=gs://dinglab/wliang_germlinevariantcalling/chrlist.txt --outputs outputPath=gs://dinglab/wliang_germlinevariantcalling/output/gatk/ --logging gs://dinglab/wliang_germlinevariantcalling/logging --disk-size datadisk:${size}" >> gcloud_command.$2.sh
done < $1
